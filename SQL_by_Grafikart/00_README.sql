...
SELECT COUNT(date) FROM recipes; -- compte les dates non NULL
...
ALTER TABLE posts RENAME TO post; -- renomme la table
ALTER TABLE posts RENAME title TO titre; -- renomme une colonne
ALTER TABLE posts ADD         content TEXT;
ALTER TABLE posts DROP COLUMN content; -- fonctionne pas




DROP TABLE posts; -- Supprimer la table
...
VALUES 
  ( 'Soupe 100', 'soupe', 'Test de contenu', 10, FALSE, 1690978150), 
  ( 'Soupe 200', 'soupe', 'Test de contenu', 10, FALSE, 1690978150); 




DELETE FROM recipes WHERE title = "Soupe 100"; -- Supprime une ligne
DELETE FROM recipes where id==2;
DELETE FROM recipes; -- Supprime tout !



UPDATE recipes SET title = "Soupe de légumes" WHERE title = "Soupe" -- Mise à jour d'un enregistrement
UPDATE recipes SET slug = "soupe100" WHERE id==3;
UPDATE recipes SET content = NULL; -- met tous les contenus à NULL
...
CREATE UNIQUE INDEX id_slug ON recipes(slug); -- Créé un index
EXPLAIN QUERY PLAN
  SELECT * FROM recipes WHERE slug=="soupe1000"; -- La recherche est plus efficace
PRAGMA index_list('recipes'); -- Supprimer un index
DROP INDEX id_slug; -- pas besoin de préciser la table l'id_slug est au niveau de la base elle même


DROP TABLE IF EXISTS categories; -- Attention à l'ordre              
DROP TABLE IF EXISTS recipes;
