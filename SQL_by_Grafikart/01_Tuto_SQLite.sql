...
SELECT * FROM recipes WHERE duration BETWEEN 25 AND 35;
SELECT * FROM recipes WHERE slug IN ('soupe', 'zoubida', 'meat');
SELECT * FROM recipes WHERE slug != 'soupe';
SELECT * FROM recipes WHERE slug == 'soupe' AND duration <= 10;
-- ILIKE insensible à la casse
SELECT * FROM recipes WHERE slug LIKE 'salade%'; 
SELECT * FROM recipes WHERE slug LIKE '%oup%';


DELETE FROM recipes WHERE title == "Soupe 200";
DELETE FROM recipes WHERE title = "Soupe 100";
...
EXPLAIN QUERY PLAN 
  SELECT * FROM recipes WHERE title == 'Soupe';
...
SELECT * FROM recipes;
UPDATE recipes SET slug = "soupe100" WHERE id==3;
UPDATE recipes SET slug = "soupe300" WHERE id==4;
SELECT * FROM recipes;
...
EXPLAIN QUERY PLAN
  SELECT * FROM recipes WHERE slug=="soupe1000";
...
/*
PRAGMA foreign_keys = ON;                        -- spécifique SQLite (voir doc)
...
INSERT 
  INTO categories (title, description)
  VALUES 
...
INSERT 
  INTO recipes (title, slug, category_id)        -- pas de content
  VALUES
    ("Crème", "creme", 2),
    ("Soupe", "soupe", 1),
    ("Salade de fruits", "salade_fruits", 2);    -- si on met 3 c'est une erreur mais ça passe
                                                 -- ajouter une contrainte (voir V2)


SELECT * FROM recipes;
SELECT * FROM recipes JOIN categories ON recipes.category_id = categories.id;
SELECT * FROM recipes r JOIN categories c ON r.category_id = c.id;              -- utilise des alias r et c
...
SELECT r.id, r.title, c.title AS "Catégorie"
FROM recipes r 
JOIN categories c ON r.category_id = c.id;              
*/
...
DROP TABLE IF EXISTS categories;                 -- Bug. Si besoin, executer cette ligne puis tout relancer
DROP TABLE IF EXISTS recipes;
...
INSERT 
  ...
  VALUES
    ("Crème", "creme", 2),
    ("Soupe", "soupe", 1),
    ("Salade de fruits", "salade_fruits", NULL);    -- Voir le NULL
...
SELECT r.id, r.title, c.title AS "Catégorie"
FROM recipes r 
INNER JOIN categories c ON r.category_id = c.id;  -- INNER = default            
...
SELECT * from recipes;      -- bizarre on voit toujours les categories 2
SELECT * from categories;   -- alors qu'elles ne sont plus dans la base categories
...
DROP TABLE IF EXISTS categories;
DROP TABLE IF EXISTS recipes;  -- On a le système de CASCADE mais en général, faire attention à l'ordre de suppression
...
SELECT recipes.title, categories.title AS "category"
FROM recipes
JOIN categories_recipes ON recipes.id = categories_recipes.recipe_id
JOIN categories         ON categories.id = categories_recipes.category_id;
...
INSERT INTO ingredients_recipes (recipe_id, ingredient_id, quantity, unit) VALUES
  ...
  (2, 4, 100, "g"),
  (2, 5, 50, "g"),
  (2, 6, 3, NULL);          -- NULL car pas d'unité 
...
SELECT recipes.title AS "Recette avec des oeufs" FROM ingredients
JOIN ingredients_recipes ON ingredients_recipes.ingredient_id = ingredients.id
JOIN recipes             ON ingredients_recipes.recipe_id = recipes.id
WHERE ingredients.name = "Oeuf";
...
SELECT *
FROM recipes
JOIN ingredients_recipes ON ingredients_recipes.recipe_id = recipes.id;   -- clé etrangère = id sur la table ciblée
...
FROM recipes
LEFT JOIN ingredients_recipes ON ingredients_recipes.recipe_id = recipes.id
WHERE ingredients_recipes.recipe_id IS NULL;
...
SELECT recipes.title AS "Recettes sans ingrédient"
FROM recipes
LEFT JOIN ingredients_recipes ON ingredients_recipes.recipe_id = recipes.id
WHERE ingredients_recipes.recipe_id IS NULL;
...
SELECT recipes.title, ingredients_recipes.quantity, ingredients_recipes.unit, ingredients.name AS "Ingrédient"
FROM recipes
JOIN ingredients_recipes ON ingredients_recipes.recipe_id = recipes.id
...
SET quantity = 10
WHERE recipe_id = 2 AND ingredient_id = 3; -- pas de clé primaire simple, faut préciser les 2
...
WHERE ingredients_recipes.recipe_id = 2;

SELECT ingredients_recipes.quantity, ingredients_recipes.unit, ingredients.name 
...
SELECT * from recipes;
SELECT COUNT(date) FROM recipes; -- 1 seule date n'est pas nulle
SELECT COUNT(*) FROM recipes;
SELECT SUM(duration) AS "Durée Totale (min.)" FROM recipes;
...
INSERT INTO ingredients_recipes (recipe_id, ingredient_id, quantity, unit) VALUES
  (3, 1, 50, "g");          -- On met que 50 gr de sucre de recette 3 

INSERT INTO ingredients (name) VALUES
  ("Miel"); -- Miel est utilisé dans aucune recette
...
SELECT ingredients.name, COUNT (id) 
FROM ingredients
LEFT JOIN ingredients_recipes ON ingredients_recipes.ingredient_id = ingredients.id
...
SELECT ingredients.name, COUNT (ingredients_recipes.recipe_id) 
FROM ingredients
LEFT JOIN ingredients_recipes ON ingredients_recipes.ingredient_id = ingredients.id
GROUP BY ingredients.name;
...
SELECT ingredients.name, COUNT (ingredients_recipes.recipe_id) AS "Count", recipes.duration 
FROM ingredients
LEFT JOIN ingredients_recipes ON ingredients_recipes.ingredient_id = ingredients.id
LEFT JOIN recipes ON ingredients_recipes.recipe_id = recipes.id
GROUP BY ingredients.name, recipes.duration;
...
FROM ingredients
LEFT JOIN ingredients_recipes ON ingredients_recipes.ingredient_id = ingredients.id
WHERE ingredients_recipes.recipe_id IS NOT NULL;
...
SELECT DISTINCT ingredients.name, recipes.duration
FROM ingredients
LEFT JOIN ingredients_recipes ON ingredients_recipes.ingredient_id = ingredients.id
LEFT JOIN recipes ON ingredients_recipes.recipe_id = recipes.id
WHERE ingredients_recipes.recipe_id IS NOT NULL;
...
-- check qu'on retrouve ce que l'on avait
SELECT ingredients.name, COUNT (ingredients_recipes.recipe_id) AS "Count"
FROM ingredients
LEFT JOIN ingredients_recipes ON ingredients_recipes.ingredient_id = ingredients.id
LEFT JOIN recipes ON ingredients_recipes.recipe_id = recipes.id
GROUP BY ingredients.name; 
...
LEFT JOIN recipes ON ingredients_recipes.recipe_id = recipes.id
GROUP BY ingredients.name 
ORDER BY Count DESC, ingredients.name ASC;
...
GROUP BY ingredients.name 
ORDER BY Count DESC, ingredients.name ASC
LIMIT 3 OFFSET 3; -- Offset == index en fait. Affiche les 3 à partir de l'index 3
...
UPDATE recipes SET date = NULL WHERE id = 2;
...
SELECT *, (
  SELECT COUNT(*) FROM ingredients_recipes WHERE recipe_id = recipes.id
) AS "Compteur" 
FROM recipes;
...
SELECT MyRecette.title
FROM (
  SELECT * FROM recipes
) AS "MyRecette";

SELECT MyRecette.MyCounter
FROM (
  SELECT COUNT(id) AS "MyCounter" FROM recipes
) AS "MyRecette";
...
SELECT *
FROM categories
WHERE categories.title = "Dessert";
...
SELECT categories_recipes.recipe_id
FROM categories
LEFT JOIN categories_recipes ON categories.id = categories_recipes.category_id -- on met à dte la clé étrangère
WHERE categories.title = "Dessert";
...
SELECT * 
FROM ingredients_recipes
WHERE ingredients_recipes.recipe_id IN (
  SELECT categories_recipes.recipe_id
  FROM categories
  LEFT JOIN categories_recipes ON categories.id = categories_recipes.category_id -- on met à dte la clé étrangère
  WHERE categories.title = "Dessert"
...
WHERE ingredients_recipes.recipe_id IN (
  ...
  FROM categories
  LEFT JOIN categories_recipes ON categories.id = categories_recipes.category_id -- on met à dte la clé étrangère
  WHERE categories.title NOT IN ("Dessert", "Gâteau")
);
...
WHERE ingredients_recipes.recipe_id IN (
  ...
  FROM categories
  LEFT JOIN categories_recipes ON categories.id = categories_recipes.category_id -- on met à dte la clé étrangère
  WHERE categories_recipes.recipe_id = ingredients_recipes.recipe_id
);
...
BEGIN TRANSACTION;
  SELECT * FROM recipes;
  DELETE FROM recipes WHERE id = 2;
  SELECT * FROM recipes WHERE Dummy = 42;
COMMIT TRANSACTION;
SELECT * FROM recipes;
...
SELECT recipes.title, GROUP_CONCAT (ingredients.name, ", ")
FROM recipes
LEFT JOIN ingredients_recipes ON ingredients_recipes.recipe_id = recipes.id
LEFT JOIN ingredients ON ingredients_recipes.ingredient_id = ingredients.id
GROUP BY recipes.id;

SELECT recipes.title, GROUP_CONCAT (ingredients.name, ", ") AS "Ingrédients"
FROM recipes
LEFT JOIN ingredients_recipes ON ingredients_recipes.recipe_id = recipes.id
LEFT JOIN ingredients ON ingredients_recipes.ingredient_id = ingredients.id
GROUP BY recipes.id;
...
CREATE VIEW recette_avec_ingredients
AS
  SELECT recipes.title, GROUP_CONCAT (ingredients.name, ", ") AS "Ingrédients"
  FROM recipes
  LEFT JOIN ingredients_recipes ON ingredients_recipes.recipe_id = recipes.id
...
SELECT * from recette_avec_ingredients;

-- Les recettes avec de la farine
SELECT * 
FROM recette_avec_ingredients
WHERE Ingrédients LIKE "%farine%";
...
WITH temp_table AS (
  SELECT id, name FROM categories2
)
SELECT name FROM temp_table; -- Y a bien 1 seul ;
...
WITH RECURSIVE temp_table AS (
  SELECT * FROM categories2 WHERE id = 14                     -- Ca c'est la ligne qui initie la récursion
  UNION ALL                                                   -- On concatène toutes les lignes : https://www.sqlitetutorial.net/sqlite-union/
  SELECT * FROM categories2 WHERE id = temp_table.parent_id   -- La seconde requete peut utiliser la vue
)
SELECT * FROM temp_table;                                     -- Une fois qu'on a la vue on l'utilise dans la requête principale

-- Marche pas car nom ambigu sur id
WITH RECURSIVE temp_table AS (
  SELECT * FROM categories2 WHERE id = 14
  UNION ALL
  SELECT * FROM categories2, temp_table WHERE id = temp_table.parent_id
)
...
WITH RECURSIVE temp_table AS (
  SELECT * FROM categories2 WHERE id = 14
  UNION ALL
  SELECT * FROM categories2, temp_table WHERE categories2.id = temp_table.parent_id
)
SELECT * FROM temp_table;

WITH RECURSIVE temp_table AS (
  SELECT id, name, parent_id FROM categories2 WHERE id = 14
  UNION ALL
  SELECT categories2.id, categories2.name, categories2.parent_id FROM categories2, temp_table WHERE categories2.id = temp_table.parent_id
)
SELECT * FROM temp_table;
...
WITH RECURSIVE temp_table AS (
  SELECT id, name, parent_id FROM categories2 WHERE id = 11  
  UNION ALL
  SELECT categories2.id, categories2.name, categories2.parent_id FROM categories2, temp_table WHERE categories2.parent_id = temp_table.id
)
SELECT * FROM temp_table;
...
WITH RECURSIVE temp_table AS (
  SELECT id, name, parent_id, 0 as "Niveau" FROM categories2 WHERE id = 11
  UNION ALL
  SELECT categories2.id, categories2.name, categories2.parent_id FROM categories2, temp_table WHERE categories2.parent_id = temp_table.id
)
SELECT * FROM temp_table;
...
WITH RECURSIVE children AS (
  ...
  FROM categories2, children WHERE categories2.parent_id = children.id
)
SELECT id, name, Niveau FROM children;


WITH RECURSIVE children AS (
  SELECT id, name, parent_id, 0 AS level, "" AS path FROM categories2 WHERE id = 11
  UNION ALL
  SELECT 
    categories2.id, 
    categories2.name, 
  ...
  FROM categories2, children 
  WHERE categories2.parent_id = children.id
)
...
WITH RECURSIVE children (id, name, parent_id, level, path) AS (
  SELECT id, name, parent_id, 0, "" FROM categories2 WHERE id = 11
  UNION ALL
  SELECT 
    categories2.id, 
    categories2.name, 
...
SELECT SUM(profit) as total FROM sales;  -- On a une seule information
...
SELECT
 *,
 SUM(profit) OVER (PARTITION BY country) -- le fenêtre est sur chaque pays
FROM sales; 

SELECT
 *,
 SUM(profit) OVER (PARTITION BY country) AS total_country,
 ROW_NUMBER() OVER (PARTITION BY country) AS row_country
FROM sales; 
 ...
 *,
 SUM(profit) OVER (PARTITION BY country) AS total_country,
 ROW_NUMBER() OVER (PARTITION BY country ORDER BY profit DESC) AS row_country
FROM sales; 
 ...
 *,
 SUM(profit) OVER (PARTITION BY country) AS total_country,
 RANK() OVER (PARTITION BY country ORDER BY profit DESC) AS rank_country
FROM sales; 
 ...
 *,
 SUM(profit) OVER (PARTITION BY country) AS total_country,
 DENSE_RANK() OVER (PARTITION BY country ORDER BY profit DESC) AS rank_country
FROM sales; 
...
FROM sales
WINDOW my_window AS (PARTITION BY country ORDER BY profit DESC);
 ...
 *,
 SUM(profit) OVER (PARTITION BY country ORDER BY Profit DESC) AS total_country,
 SUM(profit) OVER (PARTITION BY country ROWS BETWEEN CURRENT ROW AND 1 FOLLOWING) AS "total_with_next" 
FROM sales; 
...
SELECT
  ...
  SUM(profit) * 1.0 / (SELECT SUM(profit) FROM sales) AS breakdown
FROM sales
GROUP BY product
